...
def format_times_string(times_string:str):
    try:
        ...
        times_string = re.sub(r'[^0-9apmAPM:,-]', '', str(times_string))
        # Add missing comma
...
def create_time(am_pm, time_string):
    ...
    elif len(time) == 4: pass
    else: logging.warning('Invalid time: {}'.format(time))
    date = datetime.datetime.strptime(time+am_pm, '%I%M%p')
    return date

def parse_schedule_string(times_string:str):
    # return an empty array if there's no time (n/a)
    if times_string.lower().__contains__('n/a'): 
        return []
    ...
    for schedule in schedules:
        try:
            ...
            # But in case 11-1pm, 11 becomes 11 am, so it needs to be connverted to am
